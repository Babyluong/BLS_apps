// removeAmriFromQuizSessionsFixed.js - Remove AMRI AMIT from quiz_sessions table (fixed column names)
...
async function removeAmriFromQuizSessions() {
  ...
  try {
    // First, check the structure of quiz_sessions table
    console.log('1. Checking quiz_sessions table structure...');
    const { data: sampleQuiz, error: sampleQuizError } = await supabase
      .from('quiz_sessions')
      .select('*')
      .limit(1);

    if (sampleQuizError) {
      console.error('❌ Error getting sample quiz_sessions:', sampleQuizError);
      return;
    }

    if (sampleQuiz.length > 0) {
      console.log('quiz_sessions columns:', Object.keys(sampleQuiz[0]));
    }
    console.log('');

    // Check AMRI AMIT in quiz_sessions
    console.log('2. Checking AMRI AMIT in quiz_sessions...');
    ...
    console.log('3. Removing AMRI AMIT from quiz_sessions...');
    ...
    console.log('6. Testing app query after cleanup...');
    ...
    console.log('7. Checking user_id consistency...');
