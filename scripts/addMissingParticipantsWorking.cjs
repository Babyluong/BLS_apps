// scripts/addMissingParticipantsWorking.cjs
// Actually add the 8 missing participants to the database
...
// Create user profiles for missing participants
async function createUserProfiles() {
  console.log('Creating user profiles for missing participants...');
  
  for (const participant of missingParticipants) {
    try {
      // Check if profile already exists
      const { data: existing } = await makeRequest(`${SUPABASE_URL}/rest/v1/profiles?ic=eq.${participant.ic}&select=id`, {
        method: 'GET',
        headers: {
          'apikey': SUPABASE_KEY,
      ...
      const { data: authData, error: authError } = await makeRequest(`${SUPABASE_URL}/auth/v1/admin/users`, {
        method: 'POST',
        headers: {
          'apikey': SUPABASE_KEY,
          'Authorization': `Bearer ${SUPABASE_KEY}`,
      ...
      // Create profile
      const { error: profileError } = await makeRequest(`${SUPABASE_URL}/rest/v1/profiles`, {
        method: 'POST',
        headers: {
          'apikey': SUPABASE_KEY,
          'Authorization': `Bearer ${SUPABASE_KEY}`,
...
// Generate quiz sessions for missing participants
async function generateQuizSessions() {
  console.log('\nGenerating quiz sessions for missing participants...');
  
  for (const participant of missingParticipants) {
    try {
      // Get user ID by IC
      const { data: user, error: userError } = await makeRequest(`${SUPABASE_URL}/rest/v1/profiles?ic=eq.${participant.ic}&select=id`, {
        method: 'GET',
        headers: {
          'apikey': SUPABASE_KEY,
          'Authorization': `Bearer ${SUPABASE_KEY}`,
      ...
      if (userError || !user || user.length === 0) {
        console.log(`No user found for ${participant.name} (IC: ${participant.ic})`);
        continue;
      }
...
// Generate checklist results for missing participants
async function generateChecklistResults() {
  console.log('\nGenerating checklist results for missing participants...');
...
async function addMissingParticipants() {
  try {
    console.log('🚀 Starting to add missing participants to database...');
    console.log(`📊 Adding ${missingParticipants.length} participants with their test results\n`);
    
    // Step 1: Create user profiles
    await createUserProfiles();
    
    // Step 2: Generate quiz sessions
    await generateQuizSessions();
    
    // Step 3: Generate checklist results
    await generateChecklistResults();
    
    console.log('\n🎉 Successfully added all missing participants to the database!');
    console.log('✅ Check your BLS app to see the new participants with their results.');
    
  } catch (error) {
    console.error('❌ Error adding missing participants:', error);
  }
}
