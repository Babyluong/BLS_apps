...
export default function EditProfilesScreen({ onSignOut, onBack }) {
  ...
  const [hasJawatan, setHasJawatan] = useState(false); // detect if profiles.jawatan exists
  ...
  async function load() {
    ...
    try {
      if (tab === "peserta") {
        ...
        // Try selecting jawatan (preferred); if the column doesn't exist, fall back gracefully.
        const s1 = await supabase
          .from("profiles")
          .select("id, full_name, ic, email, role, jawatan")
          .eq("role","staff")
          .order("full_name",{ ascending: true });
        ...
        if (!s1.error) {
          ...
          setHasJawatan(false);
          // Normalize shape so UI code can read e.jawatan safely (null)
          setRows((Array.isArray(s2.data) ? s2.data : []).map(r => ({ ...r, jawatan: null })));
        }
      }
    } catch (e) {
  ...
  const save = async (e) => {
    try {
      if (tab === "peserta") {
        ...
        // Update staff profile. If jawatan column exists, include it; otherwise omit.
        const patch = {
          full_name: e.full_name,
          ic: e.ic,
          email: e.email || null,
        };
        if (hasJawatan) patch.jawatan = e.jawatan || null;
        ...
        {tab === "peserta" ? (
          ...
          <>
            {hasJawatan && (
              <In
                value={e.jawatan || ""}
                onChangeText={(t)=>setE({...e, jawatan:t})}
