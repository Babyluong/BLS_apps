...
export default function AddUserScreen({ onBack, onSignOut }) {
  ...
  const [jawatanTitle, setJawatanTitle] = useState("");  // e.g. "PEGAWAI PERUBATAN"
  const [jawatanGrade, setJawatanGrade] = useState("");  // e.g. "UD 10" / "N4" etc.
  ...
  const gradeOptions = useMemo(() => {
    const t = (jawatanTitle || "").toUpperCase();
  ...
  const composedJawatan = useMemo(() => {
    if (!jawatanTitle) return "";
    if (!jawatanGrade) return jawatanTitle; // grade optional
    return `${jawatanTitle} GRED ${jawatanGrade}`;
  }, [jawatanTitle, jawatanGrade]);
  ...
  }, [fullName, ic, tempat, jawatanTitle, blsYear, alergik, asma, hamil, hamilWeeks]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        jawatan: composedJawatan,             // combined title + grade
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View style={styles.card}>
          ...
          <Field label="Jawatan" required>
            <OpenPickerRow value={jawatanTitle} placeholder="Pilih jawatan" onPress={() => setPickerJawatan(true)} />
          </Field>

          <Field label="Jawatan GRED (pilihan)" required={false}>
            <OpenPickerRow value={jawatanGrade} placeholder="Pilih gred (UD/UG/UF atau mengikut kategori)" onPress={() => setPickerGrade(true)} />
            {!!composedJawatan && (
            ...
            <FlatList
              ...
              ListEmptyComponent={
                <View style={{ paddingVertical: 8 }}>
                  <Text style={{ color: "#d7ccb7", fontSize: 12 }}>Tiada gred untuk jawatan ini.</Text>
                </View>
              }
            />
          </View>
