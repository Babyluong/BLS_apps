-- createStaffAdminSQL.sql
-- SQL script to add staff and admin users to Supabase

-- 1. First, we need to create the users in the auth.users table
-- This requires using the Supabase Admin API or the Supabase Dashboard
-- For now, we'll create a script that can be run after users are created
...
-- 6. Create a view for easy access to staff and admin users
...
-- Since we can't create auth users programmatically, here are the steps:
