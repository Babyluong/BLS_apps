...
const composeJawatanGred = (jawatanRaw, gredRaw) => {
  const j = String(jawatanRaw || "").toUpperCase().trim();
...
const SYNS = {
  ...
  jawatan: ["JAWATAN","POSITION","JAWATAN PENUH"],
  gred: ["GRED","GRADE"],
...
function normalizeRow(row) {
  ...
  const jawatanOnly = bySyn(row, SYNS.jawatan) || "";
  const gred = bySyn(row, SYNS.gred) || "";
  const jawatan = composeJawatanGred(jawatanOnly, gred);
  ...
  if (!full_name || !ic || !tempat_bertugas || !jawatanOnly) {
    throw new Error("Data tidak lengkap (nama / IC / tempat / jawatan).");
  }

  return {
    full_name, ic, email: email || null, phone_number: null,
    tempat_bertugas, jawatan, bls_last_year,
  ...
  const run = async () => {
    ...
    for (let i = 0; i < workingRows.length; i++) {
      try {
        ...
        const { error } = await supabase.from("users").upsert(payload, { onConflict: "ic" });
      ...
      return (
        <View style={styles.previewRow}>
          <Text style={styles.previewTxt}>
            {index + 1}. {p.full_name} • {p.ic} • {p.tempat_bertugas} • {p.jawatan}
