...
export default function OneManCPR({ onBack, onSignOut, onNavigate }) {
  ...
  const [checklistItems, setChecklistItems] = useState({
    ...
    // RESPONSE
    'response-shoulder-tap': false,
    'response-shout': false,
    
    // SHOUT FOR HELP
    'shout-emergency': false,
    
    // AIRWAY
    'airway-head-tilt': false,
    'airway-jaw-thrust': false,
    
    // BREATHING
    'breathing-determine': false,
    'breathing-compression-begin': false,
    
    // CIRCULATION - High Quality CPR
    'circulation-location': false,
    'circulation-rate': false,
    'circulation-depth': false,
    'circulation-recoil': false,
    'circulation-minimize-interruption': false,
    'circulation-ratio': false,
    'circulation-ventilation-time': false,
    
    // DEFIBRILATION
    'defib-switch-on': false,
    'defib-attach-pads': false,
    'defib-clear-analysis': false,
    'defib-clear-shock': false,
    'defib-push-shock': false,
    'defib-resume-cpr': false,
    'defib-no-shock-continue': false,
  });
  ...
  // Define compulsory sections
  const compulsorySections = {
    airway: ['airway-head-tilt'], // Removed 'airway-jaw-thrust' from compulsory
    breathing: ['breathing-determine', 'breathing-compression-begin'],
    circulation: ['circulation-location', 'circulation-rate', 'circulation-depth', 'circulation-recoil', 'circulation-minimize-interruption', 'circulation-ratio', 'circulation-ventilation-time']
  };
  ...
  const handleSubmit = async () => {
    ...
    console.log("Selected participant:", selectedParticipant); // Debug log
    console.log("Compulsory complete:", isCompulsoryComplete()); // Debug log
    
    if (!selectedParticipant) {
      console.log("No participant selected"); // Debug log
      setSubmitResult({
    ...
    console.log("Testing Supabase connection before submit...");
    ...
    console.log("Result will be:", result); // Debug log
    ...
    try {
      ...
      const checklistDetails = {
        ...
        response_shoulder_tap: checklistItems['response-shoulder-tap'] || false,
        response_shout: checklistItems['response-shout'] || false,
        shout_emergency: checklistItems['shout-emergency'] || false,
        airway_head_tilt: checklistItems['airway-head-tilt'] || false,
        airway_jaw_thrust: checklistItems['airway-jaw-thrust'] || false,
        breathing_determine: checklistItems['breathing-determine'] || false,
        breathing_compression_begin: checklistItems['breathing-compression-begin'] || false,
        circulation_location: checklistItems['circulation-location'] || false,
        circulation_rate: checklistItems['circulation-rate'] || false,
        circulation_depth: checklistItems['circulation-depth'] || false,
        circulation_recoil: checklistItems['circulation-recoil'] || false,
        circulation_minimize_interruption: checklistItems['circulation-minimize-interruption'] || false,
        circulation_ratio: checklistItems['circulation-ratio'] || false,
        circulation_ventilation_time: checklistItems['circulation-ventilation-time'] || false,
        defib_switch_on: checklistItems['defib-switch-on'] || false,
        defib_attach_pads: checklistItems['defib-attach-pads'] || false,
        defib_clear_analysis: checklistItems['defib-clear-analysis'] || false,
        defib_clear_shock: checklistItems['defib-clear-shock'] || false,
        defib_push_shock: checklistItems['defib-push-shock'] || false,
        defib_resume_cpr: checklistItems['defib-resume-cpr'] || false,
        defib_no_shock_continue: checklistItems['defib-no-shock-continue'] || false
      };
      ...
      console.log("One Man CPR Assessment saved:", savedResult);
  ...
  const ChecklistItem = ({ id, text, subItems = [], isCompulsory = false }) => {
      ...
      {subItems.length > 0 && (
        <View style={styles.subItemsContainer}>
            {subItems.map((subItem, index) => {
              const isSubItemCompulsory = Object.values(compulsorySections).some(section => section.includes(subItem.id));
              return (
                <View key={`${id}-sub-${index}`} style={[styles.subItemContainer, isSubItemCompulsory && styles.checklistItemCompulsory]}>
                  <View style={styles.subTextContainer}>
              <Text style={[styles.subItemText, checklistItems[subItem.id] && styles.completedText]}>
                {subItem.text}
              </Text>
                    {isSubItemCompulsory && (
                      <Text style={styles.compulsoryLabel}>REQUIRED</Text>
                  ...
                  <Switch
                    value={checklistItems[subItem.id]}
                    onValueChange={() => toggleItem(subItem.id)}
                    trackColor={{ false: "#767577", true: "#4CAF50" }}
                    thumbColor={checklistItems[subItem.id] ? "#ffffff" : "#f4f3f4"}
                    ios_backgroundColor="#3e3e3e"
                  />
                </View>
              );
  ...
  return (
    <LuxuryShell title="One Man CPR Checklist" onSignOut={onSignOut} onBack={handleBackToMenu}>
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.participantSection}>
          ...
          {loading ? (
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                // This will be handled by a custom modal
        ...
        <View style={styles.section}>
          ...
          <ChecklistItem 
            id="danger-ppe"
            text="Wear PPE (gloves, apron, mask), look out for hazard"
          />
        </View>
        ...
        <View style={styles.section}>
          ...
          <ChecklistItem 
            id="response-shoulder-tap"
            text="a. Shoulder tap"
          />
          <ChecklistItem 
            id="response-shout"
            text='b. Shout & speak "are you okay?"'
          />
        </View>
        ...
        <View style={[styles.section, styles.sectionCompulsory]}>
          ...
          <ChecklistItem 
            id="breathing-determine"
            text="a. Determine while opening the airway by looking at the chest, in not more than 10 seconds (and if you are trained, simultaneously feel for the presence of pulse)."
          />
          <ChecklistItem 
            id="breathing-compression-begin"
            text="b. Chest compression shall begin with absence of normal breathing or no pulse."
          />
        </View>
        ...
        <View style={[styles.section, styles.sectionCompulsory]}>
          ...
          <Text style={styles.subsectionTitle}>Performed high quality of CPR</Text>
          <ChecklistItem 
            id="circulation-location"
            text="i. Location - middle of chest, lower half of sternum"
          />
          <ChecklistItem 
            id="circulation-rate"
            text="ii. Rate of compression: 100-120/min"
          />
          <ChecklistItem 
            id="circulation-depth"
            text="iii. Depth of compression: 5-6 cm"
          />
          <ChecklistItem 
            id="circulation-recoil"
            text="iv. Full recoil after each compression"
          />
          <ChecklistItem 
            id="circulation-minimize-interruption"
            text="v. Minimize Interruption"
          />
          <ChecklistItem 
            id="circulation-ratio"
            text="vi. Compressions to ventilations ratio, 30:2"
          />
          <ChecklistItem 
            id="circulation-ventilation-time"
            text="vii. Each ventilation in 1 second"
          />
        </View>

        {/* DEFIBRILATION Section */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>DEFIBRILATION</Text>
          <Text style={styles.subsectionTitle}>
            As soon as the AED arrives, or if one is already available at the site of the cardiac arrest:
          </Text>
          <ChecklistItem 
            id="defib-switch-on"
            text="a. Switch on the AED and follow voice prompt"
          />
          <ChecklistItem 
            id="defib-attach-pads"
            text="b. Attach the electrode pads"
          />
          <ChecklistItem 
            id="defib-clear-analysis"
            text="c. Clear the victim during rhythm analysis"
          />
          <ChecklistItem 
            id="defib-clear-shock"
            text='d. If shock is advised, i. Clears the victim and loudly state "Stand Clear"'
          />
          <ChecklistItem 
            id="defib-push-shock"
            text="d. If shock is advised, ii. Push shock button as directed"
          />
          <ChecklistItem 
            id="defib-resume-cpr"
            text="d. If shock is advised, iii. immediately resume CPR"
          />
          <ChecklistItem 
            id="defib-no-shock-continue"
            text="e. If no shock is indicated, continue CPR"
          />
        </View>
