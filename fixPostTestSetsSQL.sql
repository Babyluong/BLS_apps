-- fixPostTestSetsSQL.sql
-- Fixed version with proper type casting for PostgreSQL
...
BEGIN
    ...
    LOOP
        ...
        IF checklist_data.checklist_details IS NOT NULL THEN
            ...
            percentage := CASE 
                WHEN total_count > 0 THEN ROUND((performed_count::numeric / total_count) * 100)
                ELSE 0
            END;
...
-- 12. Create function to get posttest statistics by set (FIXED)
...
BEGIN
    ...
    SELECT 
        ...
        ROUND(AVG(qs.score::numeric), 2) as average_score,
        ROUND(
            (COUNT(CASE WHEN qs.score >= 21 THEN 1 END)::numeric / COUNT(*)::numeric) * 100, 
            2
        ) as pass_rate
...
-- 13. Create function to get user's complete assessment including posttest set (FIXED)
...
BEGIN
    ...
    SELECT 
        ...
        qs.score,
        qs.total_questions,
        ROUND((qs.score::numeric / qs.total_questions::numeric) * 100, 2) as percentage,
...
-- 15. Create view for easy querying of posttest sets (FIXED)
