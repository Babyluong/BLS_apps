    ...
    try {
      if (tab === "peserta") {
        ...
        const mapped = (Array.isArray(data) ? data : []).map((r) => ({
          ...
          jawatan: r.jawatan || r.job_position || r.gred || "—",
          source: "profiles",
        }));
        setRows(mapped);
      }
  ...
  const doDelete = async () => {
    ...
    try {
      if (selected.source === "users") {
        const { error: delErr } = await supabase.from("users").delete().eq("id", selected.id);
        ...
        if (selected.email || selected.ic) {
          try {
            ...
            if (selected.email) orParts.push(`email.eq.${selected.email}`);
            if (selected.ic) orParts.push(`ic.eq.${selected.ic}`);
            if (orParts.length > 0) await supabase.from("users").delete().or(orParts.join(","));
          } catch {}
        }
      }
